...
import { getFeedback, updateFeedback } from "../../../connector";
...
const AdminFeedback = () => {
    ...
    return <main className="management-page"><div className="management-shell">
        ...
        <section className="management-list feedback-list">{items.length ? items.map(item => <article className="management-item feedback-item" key={item.id}>
            ...
            <div className="management-actions">{item.status === "unread" && <button onClick={() => setStatus(item, "read")}>Mark read</button>}{item.status !== "resolved" && <button className="primary" onClick={() => setStatus(item, "resolved")}>Resolve</button>}</div>
